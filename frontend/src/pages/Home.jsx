...
// Flow: Pilih sumber -> Ambil/Upload gambar -> Preview -> Scan via API -> Lihat hasil -> Accept/Decline
...
/**
 * dataURLtoFile - Konversi data URL (dari kamera) ke File object
 */
...
export default function Home() {
    ...
    /**
     * handleImageCapture - Callback ketika gambar diambil dari kamera (data URL)
     */
    ...
    /**
     * handleFileSelect - Callback ketika file dipilih via upload
     * ImageUpload sekarang mengirim data URL, kita convert ke File
     */
    const handleFileSelect = useCallback((imageDataUrl) => {
        setCapturedImage(imageDataUrl);
        const file = dataURLtoFile(imageDataUrl, `upload_${Date.now()}.jpg`);
        setImageFile(file);
        setCurrentState(STATES.PREVIEW);
    }, []);

    /**
     * handleStartScan - Memulai proses scanning via API
     */
    ...
    /**
     * handleAccept - User menerima hasil scan (sudah tersimpan di server)
     */
    const handleAccept = useCallback(() => {
        // Data sudah tersimpan di server saat scan, langsung reset
        resetToStart();
    }, []);
    ...
    const handleDecline = useCallback(() => {
        // TODO: Bisa tambahkan API call untuk hapus record terakhir jika diperlukan
        resetToStart();
    }, []);
    ...
    return (
        <div className="min-h-screen">
            ...
            {/* State: Scanning via API */}
            {currentState === STATES.SCANNING && imageFile && (
                <div className="container-app">
                    <ScanProgress
                        imageFile={imageFile}
